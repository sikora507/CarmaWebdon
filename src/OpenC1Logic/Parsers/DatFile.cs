...
namespace OpenC1Logic.Parsers
{
    ...
    public class DatFile : BaseDataFile
    {
        ...
        {
            if (filename.EndsWith(".ACT", StringComparison.OrdinalIgnoreCase))
            ...
            while (true)
            {
                ...
                switch (type)
                {
                    ...
                    case (int)BlockType.ModelName:
                        ...
                        if (deformMainModel && Path.GetFileNameWithoutExtension(name).Equals(Path.GetFileNameWithoutExtension(filename), StringComparison.OrdinalIgnoreCase))
